...
export default function ProductCard({ product }) {
  const { addToCart } = useCart(); const { isAuthenticated } = useAuth(); const navigate=useNavigate();
  const [wishlisted,setWishlisted]=useState(false); const [wishLoading,setWishLoading]=useState(false);
  const firstVariant=product.variants?.[0]; const displayPrice=firstVariant?.price??0; const originalPrice=firstVariant?.original_price??null;
  const image=product.main_image_url||product.images?.find(i=>i.is_primary)?.image_url||product.images?.[0]?.image_url||null;
